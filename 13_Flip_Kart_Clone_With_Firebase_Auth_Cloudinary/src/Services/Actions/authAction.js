...
export const signINAsync = ({ email, password }) => async (dispatch) => {
  try {
    ...
    if (userSnap.exists()) {
      dispatch(signINSuc({ ...userSnap.data(), uid: res.user.uid }));
    } else {
      const user = {
        id: res.user.uid,
        ...
        displayName: "User",
        role: "user",
        photoURL: res.user.photoURL || "https://media.istockphoto.com/id/1300845620/vector/user-icon.jpg",
      };
      await setDoc(userRef, user);
      dispatch(signINSuc({ ...user, uid: user.id }));
    }
  } catch (err) {
  ...
  try {
    ...
    if (userSnap.exists()) {
      ...
      user = {
        ...
        photoURL: googleUser.photoURL || "https://media.istockphoto.com/id/1300845620/vector/user-icon.jpg",
      };
      await setDoc(userRef, user);
    }
...
export const checkAuthStateAsync = () => (dispatch) => {
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      const userSnap = await getDoc(userRef);
      if (userSnap.exists()) {
        dispatch(signINSuc({ ...userSnap.data(), uid: user.uid }));
      } else {
        dispatch(signOUTSUC());
      }
    } else {
